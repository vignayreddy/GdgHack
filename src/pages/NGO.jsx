...
export default function NGO() {
  ...
  useEffect(() => {
    ...
    const unsubMoods = onSnapshot(collection(db, 'moods'), (snap) => {
      ...
      const moodValues = { 'Great': 10, 'Okay': 5, 'Not Good': 2 };
      ...
      // Update daily mood counts (simplified for demo)
    ...
    <div className="space-y-12 pb-20">
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="space-y-2">
          <h1 className="serif text-4xl font-bold text-brand-text">NGO Wellbeing Dashboard</h1>
          <p className="text-brand-muted">Data-driven insights for organizations to monitor and act.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          >
            ...
            <div>
              <div className="text-brand-muted text-xs font-bold uppercase tracking-wider">{stat.label}</div>
              <div className="serif text-2xl font-bold text-brand-text">{stat.value}</div>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="card p-8 space-y-6">
          <h2 className="serif text-2xl font-bold text-brand-text">Mood Check-ins (Daily)</h2>
        ...
        <div className="card p-8 space-y-6">
          <h2 className="serif text-2xl font-bold text-brand-text">Issues Distribution</h2>
            ...
            <div className="space-y-2">
              {categoryData.map((entry, index) => (
                <div key={index} className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-brand-muted">{entry.name}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="card p-8 space-y-6 lg:col-span-2">
          <h2 className="serif text-2xl font-bold text-brand-text">Overall Wellbeing Index</h2>
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
